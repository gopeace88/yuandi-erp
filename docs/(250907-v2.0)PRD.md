...
| **order_manager** | 주문 관리만 | 주문 생성/수정, 재고 관리, 출납장부 조회 |
| **ship_manager** | 배송 관리만 | 송장 등록, 출고 처리, 출납장부 조회 |
...
cashbook_transactions {
  ...
  type: enum('SALE', 'PURCHASE', 'REFUND', 'ADJUSTMENT') NOT NULL,
  amount: decimal NOT NULL,
  currency: enum('CNY', 'KRW') DEFAULT 'KRW',
  exchange_rate: decimal,
  amount_krw: decimal NOT NULL,
...
inventory_logs {
  id: uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  product_id: uuid REFERENCES products(id),
  type: enum('INBOUND', 'SALE', 'ADJUSTMENT', 'RETURN') NOT NULL,
  quantity: integer NOT NULL, -- +입고, -출고
  unit_cost: decimal,
