...
**핵심 기능**: 
- 시스템 설정 (카테고리, 출납유형 관리)
- 상품 등록 및 재고 관리 (Product Registration & Inventory Management)
- 주문 관리 시스템 (Order Management)
- 배송 관리 (Shipment Management)
...
상품 등록 화면:
1. 상품명 입력 (한국어/중국어)
2. 카테고리 선택 (1단계에서 등록한 카테고리)
3. 모델명, 색상, 브랜드 입력
4. 가격 정보 입력 (원가/판매가)
5. 초기 재고 수량 입력
6. products 테이블에 INSERT
7. SKU 자동생성: [카테고리]-[모델]-[색상]-[브랜드]-[HASH5]
```

### 2.3 재고 입고 플로우
```
재고 입고 화면:
1. 상품 선택 (2단계에서 등록한 상품)
2. 입고 수량 입력
3. 입고 단가 입력 (선택)
4. products.onHand 증가
5. inventory_movements 기록 (입고 이력)
6. cashbook 기록 (1단계에서 설정한 출납유형 사용)
```
...
### 2.7 환불 플로우 (SHIPPED/DONE → REFUNDED)
```
배송 후 환불:
...
### 4.5 상품 등록 화면 (admin, order_manager)
**상품 등록**:
- 상품명 입력 (한국어/중국어)
- 카테고리 선택 (시스템 설정에서 등록한 카테고리)
- 모델명, 색상, 브랜드 입력
...
### 4.6 재고 관리 화면 (admin, order_manager)
**재고 입고**:
- 상품 선택 (등록된 상품 중 선택)
- 입고 수량 및 단가 입력
- 재고 수량 자동 증가
...
### 4.7 배송 관리 화면 (admin, ship_manager)
