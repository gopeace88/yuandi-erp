...
async function seedCompleteFlow() {
  ...
  try {
    ...
    for (let i = 1; i <= 200; i++) {
      ...
      // Determine status (30% PAID, 30% SHIPPED, 20% DONE, 10% CANCELLED, 10% REFUNDED)
      let status
      ...
      // Update stock for non-cancelled orders
      if (status !== 'CANCELLED') {
    ...
    // ========================================
    // STEP 3: Process order statuses and create related records
    // ========================================
    console.log('\n⚙️ Step 3: Processing order statuses...')
    
    const shipments = []
    
    // Update product stock and create related records
    for (const order of createdOrders) {
      const product = createdProducts.find(p => p.id === order.product_id)
      
      // Inventory movement for sale (except CANCELLED)
      if (order.status !== 'CANCELLED') {
        orderMovements.push({
          ...
          quantity: -1,
          unit_cost: product.cost_cny,
          balance_before: productStock[order.product_id] + 1,
          balance_after: productStock[order.product_id],
          ref_type: 'order',
          ref_id: order.id,
        ...
        await supabase
          .from('products')
          .update({ on_hand: productStock[order.product_id] })
